...
The plugin owns a note's frontmatter and, since the description field, exactly one section of its body. `calendars/note_body.ts` reads and rewrites the text under a named heading — `# DESCRIPTION` — and is pure and unit-tested for the same reasons `frontmatter.ts` is. The rule is the same one too: everything outside the section passes through in place and in order, because a user's note may hold anything and none of it was put there by this plugin. The body is read **lazily**, only when the edit modal opens on an event, so a description never enters `FerryEvent`, the cache, the ICS export or a notification — startup still touches nothing but the metadata cache.
